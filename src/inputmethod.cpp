...
void CCoreEventCallback::on_process_input_device_event(sclu32 &type, sclchar *data, size_t &len, sclu32 *ret)
{
    ...
    if (g_event_callback.process_input_device_event) {
        void *input_data = static_cast<void*>(data);
...
int ime_input_device_rotary_get_direction(ime_input_device_event_h device_event, ime_input_device_rotary_direction_e *direction)
{
    ime_error_e retVal = IME_ERROR_NONE;

    if (!device_event || !direction) {
        LOGW("IME_ERROR_INVALID_PARAMETER");
        return IME_ERROR_INVALID_PARAMETER;
    }
    ...
    Ecore_Event_Detent_Rotate *rotary_device_event = static_cast<Ecore_Event_Detent_Rotate*>(device_event);
    *direction = rotary_device_event->direction;

    return IME_ERROR_NONE;
}

int ime_input_device_rotary_get_timestamp(ime_input_device_event_h device_event, unsigned int *timestamp)
{
    ime_error_e retVal = IME_ERROR_NONE;
