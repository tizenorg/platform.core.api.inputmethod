...
void CCoreEventCallback::on_process_input_device_event(sclu32 &type, sclchar *data, size_t &len, sclu32 *ret)
{
    typedef struct {
        int ecore_event_id;
        ime_input_device_type_e dev_type;
    } ime_dev_type_conv_table;

    ime_dev_type_conv_table conv_table[] = {
        { ECORE_EVENT_DETENT_ROTATE , IME_INPUT_DEVICE_TYPE_ROTARY },
    };

    ime_input_device_type_e dev_type = IME_INPUT_DEVICE_TYPE_NONE;
    for (unsigned int loop = 0; loop < sizeof(conv_table) / sizeof(ime_dev_type_conv_table); loop++) {
        if ((unsigned int)(conv_table[loop].ecore_event_id) == type) {
            dev_type = conv_table[loop].dev_type;
        }
    }
    ...
    if (g_event_callback.process_input_device_event) {
        ...
        bool processed = g_event_callback.process_input_device_event(dev_type, input_data, user_data);
